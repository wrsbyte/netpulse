...
from netpulse.api.raw_queries import RawQuery, raw_page
...
def test_aggregates_run_over_the_filtered_set(tmp_path: Path) -> None:
    _seed(tmp_path)
    with get_session() as s:
        page = raw_page(s, MODEL, SPECS, RawQuery(window=3600, network_id=None), 100, 0)
    assert page.total == 3
...
def test_filter_narrows_rows_and_aggregates(tmp_path: Path) -> None:
    ...
    with get_session() as s:
        ...
        page = raw_page(s, MODEL, SPECS, rq, 100, 0)
    assert page.total == 2
    rtt = next(a for a in page.agg if a.column == "rtt_avg")
